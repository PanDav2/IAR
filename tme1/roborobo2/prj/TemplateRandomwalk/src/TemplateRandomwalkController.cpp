...
TemplateRandomwalkController::TemplateRandomwalkController( RobotWorldModel *__wm ) : Controller ( __wm )
{
    ...
    {
        _params.push_back(ranf()*2.0-1.0);
    }
}
...
void TemplateRandomwalkController::monitorSensoryInformation()
{
    ...
    if ( gVerbose && gDisplayMode <= 1 && gRobotIndexFocus == _wm->getId() )
    {
        
        std::cout << "=-= Robot #" << _wm->getId() << " : STARTING monitoring sensory information\n";
        ...
        for(int i  = 0; i < _wm->_cameraSensorsNb; i++)
        {
            ...
            //double distanceNormalized = _wm->getDistanceValueFromCameraSensor(i) / _wm->getCameraSensorMaximumDistanceValue(i); // Similar to _wm->getNormalizedDistanceValueFromCameraSensor(i);
